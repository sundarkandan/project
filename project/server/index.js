        ...
        if (existingHTML && existingHTML.trim() !== "") {
            systemInstruction = `
               ${scrollbarStyle}
                ...
                TASK:
                1. Modify the EXISTING CODE based on the provided CURRENT USER REQUEST.
                ...
                6. ensure all the user requests are worked perfectly
          
            `; 
        } else {
            systemInstruction = `
            ${scrollbarStyle}
                Prompt: ${prompt}. Build a single page website using the provided prompt.
                RULES:
                ...
                5. use internal javascript is needed for UI elements, otherwise ignore it.
                6. If the website has scrolling content, you MUST apply the professional slim scrollbar styling using the provided CSS.
                7. If the content has navigation must all the navigations are works properly 
                8. If the content is a single-page layout without overflow, the page must be designed to fit the screen dimensions perfectly ($100vh$ and $100vw$)`
        }
...
// --- Backend save-project endpoint-la intha check-ah update pannunga ---
app.post("/save-project", async (req, res) => {
    try {
        const { email, prompt, html, projectId, chatHistory } = req.body; 
        
        const finalPrompt = prompt || (chatHistory && chatHistory.length > 0 ? chatHistory[0].text : "Untitled Project");

        let project;
        // FIX: projectId 'null' or 'undefined' (string-ah kooda) varuthanu check panrom
        if (projectId && projectId !== "null" && projectId !== "undefined" && mongoose.Types.ObjectId.isValid(projectId)) {
            ...
            // Forcefully existing project irukanu email vachi check pannalam (Optional safety)
            project = new Project({ 
                userEmail: email, 
                prompt: finalPrompt, 
                html, 
        ...
        res.status(200).json({ success: true, message: "Project synced!", projectId: project._id });
    } catch (err) {
