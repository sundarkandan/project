...
const Workspace = () => {
  ...
  const [searchTerm, setSearchTerm] = useState(""); // 1. Search state added
...
const handleNewProject = () => {
  // 1. Clear all session data for a fresh start
  localStorage.removeItem("activeProject");
  localStorage.removeItem("lastGeneratedHTML");
  localStorage.removeItem("chatHistory");

  // 2. Navigate with empty state to ensure no leftover data
  navigate("/Aibuilder", { state: null });
};
  // 2. Filter logic based on searchTerm
    ...
    <div className="dark text-slate-200 overflow-hidden h-screen flex bg-[#020617] font-inter">
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="h-16 border-b border-slate-800 flex items-center justify-between px-8 bg-slate-900/80 backdrop-blur">
          {/* 3. Search input linked to state */}
          <div className="relative w-full max-w-sm">
            <input 
              ...
              className="w-full bg-slate-900 border border-slate-800 rounded-full py-1.5 px-10 text-sm focus:outline-none focus:border-cyan-500/50" 
            />
            <svg className="w-4 h-4 absolute left-3.5 top-2.5 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex items-center gap-4">
            ...
            <button onClick={handleNewProject} className="bg-cyan-500 hover:bg-cyan-400 text-slate-950 px-4 py-1.5 rounded-md text-sm font-bold shadow-[0_0_15px_rgba(34,211,238,0.4)] transition-all">New Project</button>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-white">
              {searchTerm ? `Search Results (${filteredProjects.length})` : "Recent Projects"}
            </h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {!searchTerm && (
              <div onClick={handleNewProject} className="bg-slate-900 border border-slate-800 rounded-xl h-64 flex flex-col items-center justify-center text-slate-500 hover:border-cyan-500/30 hover:text-cyan-400 transition-all cursor-pointer group">
                <span className="text-4xl mb-2 group-hover:scale-110 transition-transform">+</span>
                <p className="text-sm font-medium">Create Project</p>
              </div>
            )}

            {/* 4. Mapping over filteredProjects instead of projects */}
...
{filteredProjects.map((project) => (
  <div 
    ...
    onClick={() => {
      ...
      navigate("/Aibuilder", { state: { savedProject: project } });
    }} 
  ...
  >
    {/* ... (Bakki ullathellam apdiye irukattum) */}
                <button 
